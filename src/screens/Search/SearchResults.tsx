...
let SearchResults = ({
  ...
  const {t: l} = useLingui()
  const activePage = hasFilters && activeTab > 1 ? 0 : activeTab
  const tabShape = hasFilters ? 'filtered' : 'plain'

  const sections = useMemo(() => {
    if (!query && !hasFilters) return []
    // People and Feeds tabs only make sense for a plain text query - structured
    // post filters don't apply to them.
  ...
  }, [l, query, queryWithParams, filters, hasFilters, activePage])
...
let SearchScreenPostResults = ({
  ...
  const v1 = useSearchPostsQuery({
    ...
    sort,
    enabled: active && !advancedSearchV2Enabled,
  })
